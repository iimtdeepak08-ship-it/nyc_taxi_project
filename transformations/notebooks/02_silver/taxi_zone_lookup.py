...
# Updating record for LocationID 1
df = df.withColumn("borough", when(col("LocationID")==1, "NEWARK AIRPORT").otherwise(col("borough")))
...
dt.alias("t").\
    merge(
        source    = df.alias("s"),
        condition = "t.LocationID = s.LocationID AND t.end_date IS NULL AND (t.borough != s.borough OR t.zone != s.zone OR t.service_zone != s.service_zone)"
    ).\
    whenMatchedUpdate(
...
insert_id_list = [row.LocationID for row in dt.toDF().filter(f"end_date = '{end_timestamp}' ").select("LocationID").collect()]
...
if len(insert_id_list) == 0:
    ...
    dt.alias("t").\
        merge(
            source    = df.alias("s"),
            condition = f"s.LocationID not in ({', '.join(map(str, insert_id_list))})"
        ).\
        whenNotMatchedInsert(
            values = { "t.LocationID": "s.LocationID",
                    "t.borough": "s.borough",
                    "t.zone": "s.zone",
...
dt.alias("t").\
    merge(
        source    = df.alias("s"),
        condition = "t.LocationID = s.LocationID"
    ).\
    whenNotMatchedInsert(
        values = { "t.LocationID": "s.LocationID",
                "t.borough": "s.borough",
