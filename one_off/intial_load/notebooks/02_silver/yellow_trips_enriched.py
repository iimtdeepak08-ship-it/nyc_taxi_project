...
display(df_trips)
display(df_zones)
# DBTITLE 1,Create a view for the taxi zone lookup table
#
...
df_join_1 = df_trips.join(
    df_zones,
    df_trips.pu_location_id==df_zones.LocationID,"left"
    ).select(
        df_trips.vendor,
...
df_join_final = df_join_1.join(df_zones,df_join_1.do_location_id==df_zones.LocationID,"left").select(
    df_join_1.vendor,
